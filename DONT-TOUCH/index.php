<?php
	session_start();
                          
	if($_SESSION['sid']!=session_id())
	{
                header("location:login.php");
		
	}
?>

<html>
<head>
 <meta charset="UTF-8"> 
  <link rel="shortcut icon" href="favicon.ico">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <link href='https://fonts.googleapis.com/css?family=Raleway:400,300,600' rel='stylesheet' type='text/css'>
  <link href='st.css' rel='stylesheet' type='text/css'>
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js"></script>
  <script src="https://cdn.rawgit.com/showdownjs/showdown/1.6.3/dist/showdown.min.js"></script>
  <script type="text/javascript" src='rrr.js'></script>
  <script src="js/js.cookie.js"></script>
  <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" integrity="sha384-BVYiiSIFeK1dGmJRAkycuHAHRg32OmUcww7on3RYdg4Va+PmSTsz/K68vbdEjh4u" crossorigin="anonymous">
  <link rel="stylesheet" href="css/carousel.css">
   <link href="css/navbar-fixed-top.css" rel="stylesheet">
   <link href="css/footer.css" rel="stylesheet" type="text/css">
   <link href="css/blog.css" rel="stylesheet" type="text/css">
   <title>LIFEGROUPS: House of Bread Church</title>


</head>
<body>



<div class='markup'>
  <div class='navigation'></div>
  <div class='view-container container'> </div>
</div>


<div class='templates'>
	<div class='ruweek-view'>
		<div class='container'>
			<div class="video-container" data-name="video"></div>
		</div>
		<div class="blog-main">
			<div  class="blog-header">
        			<h1 class="page-title" >Неделя: &#x2116; 
					<span class="page-title" data-name="week"></span>
				</h1>
				<h1  class="blog-title" data-name="topic"></h1>
			</div>
			<div class="page-header">

				<p class="blog-post-meta" data-name="speaker"></p>
				<p class="lead blog-description" data-name="lead"></p>
			</div>
			<div id='post' class="blog-post"></div><!-- /.blog-post -->
		</div><!--// blog-main //-->   
       </div><!--// ruweek-view //-->
</div><!--// templates //-->

<div class='templates'>
		<div class='enweek-view'>
			<div class='container'><br><br>
				<div class="video-container" data-name="evideo"></div>
			</div>
			<div class="blog-main">

				<div  class="blog-header">
        			<h1 class="page-title" >Week: #
					<span class="page-title" data-name="week"></span>
				</h1>
				<h1  class="blog-title" data-name="etopic"></h1>
			</div>
			<div class="page-header">

				<p class="blog-post-meta" data-name="espeaker"></p>
				<p class="lead blog-description" data-name="elead"></p>
			</div>
			<div id='post' class="blog-post"></div>
		</div><!--// blog-main //-->   
       </div><!--// enweek-view //-->
</div><!--// templates //-->


<div class='templates'>
    <div class='404' >
	    <div class="cover-container">
                        <div class='cover'>
                        	<img src='i/404.jpg'>
                        </div>
                          <div class="inner cover">
                            <h1 class="cover-heading"><span class="glyphicon glyphicon-ban-circle" aria-hidden="true"></span></h1>
                            <h1 class="cover-heading">Nothing  in here. <br>Seems like some parts of the web are out of ink right now.</h1>
                            <p class="lead">The best members of our team are working on it.</p>
                            <p class="lead">Please come here later.</p>
                            <p class="lead">
                              <a href="#" class="btn btn-lg btn-default">Let's us bring you home</a>
                            </p>
                          </div>
		</div><!--// blog-main //-->   
       </div><!--// enweek-view //-->
</div><!--// templates //-->

<footer class="footer"></footer>


    <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>
    <script>window.jQuery || document.write('<script src="../../assets/js/vendor/jquery.min.js"><\/script>')</script>
    <script src="js/bootstrap.min.js"></script>
    <!-- Just to make our placeholder images work. Don't actually copy the next line! -->
    <script src="js/vendor/holder.min.js"></script>
    <!-- IE10 viewport hack for Surface/desktop Windows 8 bug -->
    <script src="js/ie10-viewport-bug-workaround.js"></script>

  <script type='text/javascript'>
    $(window).ready(rrr.appOnReady());
  </script>
</body>
</html>
